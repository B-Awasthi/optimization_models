...
# below data is added to food_manufacture_2
maximum_ingredients = 3
minimum_consumption = 20

# model instantiation
model = pywraplp.Solver(
    "food_manufacture_2", pywraplp.Solver.CBC_MIXED_INTEGER_PROGRAMMING
)
...
# 5. oil usage
dv_oil_used = {i: model.BoolVar(str(i)) for i in month_oil}
...
#    binary variable for each product and each month is set to one if and
#    only if the continuous variable used for the same product and the
#    same month is non-zero.
big_M = 10000
for mth in months:
    for ol in oils:
        model.Add(dv_oil_consume[mth, ol] - big_M * dv_oil_used[mth, ol] <= 0)
        model.Add(
            dv_oil_consume[mth, ol] - minimum_consumption * dv_oil_used[mth, ol] >= 0
        )

# 8. cannot use more number of oils than maximum_ingredients in each month
for mth in months:
    model.Add(model.Sum(dv_oil_used[mth, ol] for ol in oils) <= maximum_ingredients)

# 9. if vegetable one or vegetable two are used, then oil three must also be used
for mth in months:
    model.Add(dv_oil_used[mth, "VEG1"] <= dv_oil_used[mth, "OIL3"])
    model.Add(dv_oil_used[mth, "VEG2"] <= dv_oil_used[mth, "OIL3"])
